/**
 * Stage 2 — text extraction, PURE CORE.
 *
 * Turns a loaded pdf.js document into a normalized, layout-aware line model. This
 * module is deliberately free of any `unpdf` / `node:` import so it can run in BOTH
 * environments: the server wraps it with unpdf's `getDocumentProxy` (see
 * extract.ts), and the browser wraps it with the bundled `unpdf/pdfjs` renderer
 * (see client-parse.ts) to parse oversized PDFs without an upload. The only
 * reference to pdf.js here is a TYPE import, which is erased at compile time.
 *
 * Coordinates are normalized to be TOP-BASED (y increases downward), which is how
 * the parser reasons about reading order.
 */
 ...
 *  split the file, rather than silently risking the time/parse budget. */
...
/**
 * Build the line model from an ALREADY-LOADED pdf.js document. Engine-agnostic:
 * the caller supplies the proxy (unpdf on the server, unpdf/pdfjs in the
 * browser), and this never touches the network, filesystem, or node built-ins.
 */
export async function extractFromProxy(
  pdf: PDFDocumentProxy,
  opts?: { maxPages?: number },
): Promise<ExtractedDoc> {
  // Guard the budget up front: numPages is known after load but before the
