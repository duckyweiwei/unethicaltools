...
/**
 * Video converter hub. This is the page the old `/converters` route
 * served. `/converters` now hosts the multi-type chooser (Video / Audio
 * / Image), so the video-specific listing moved here to match the
 * `/audio-converter` and `/image-converter` naming.
 */
export const metadata: Metadata = {
  title: `Video Converter — Free, Private, No Upload · ${SITE_NAME}`,
  description:
    "Every video format converter on unethical tools, one page. Convert TS, MOV, MKV, WEBM, AVI, FLV, or MPEG to MP4. Runs in your browser — no upload, no account, no quota.",
  alternates: { canonical: absoluteUrl("/video-converter") },
  openGraph: {
    type: "website",
    url: absoluteUrl("/video-converter"),
...
export default function VideoConverterHubPage() {
  return (
    <>
      <StructuredData
        data={[
